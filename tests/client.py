...
class JWTClient:
    ...
    def list(self):
        ...
        headers = self.get_headers()
        data = {
            "text": "Would you like to have some coffee?",
        }
        audio_file = open("../speech_main/test7.wav", "rb")
        url = "http://localhost:8000/api/input/"
        r = requests.post(url, data=data, files={
                          "audio_file": audio_file}, headers=headers)
        data = r.json()
        return data
